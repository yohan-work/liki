...
Hermes Agent는 NousResearch가 만든 오픈소스 AI 에이전트 프레임워크로 소개된다. [[WikiDocs Hermes Agent Guide]]는 Hermes Agent를 설치, CLI, 모델 연결, 도구, 메모리, 컨텍스트, 스킬, MCP, 메시징 게이트웨이, 반복 실행, 보안, 실전 프로젝트를 포함하는 agent runtime으로 다룬다. [[Blake Crosley Hermes Agent Guide]]는 Hermes를 채팅 래퍼가 아니라 CLI, messaging gateway, tool execution, persistent memory, skill, cron, MCP, profile을 묶은 agent runtime으로 더 상세히 설명한다. [[YOZM Hermes Agent Article]]은 Hermes의 self-improving을 “곧바로 똑똑해진다”보다 “쓸수록 사용자 workflow에 맞춰진다”는 관점으로 해석한다.
...
- 현재 이 페이지는 WikiDocs landing page와 practitioner guide를 근거로 하므로, 공식 문서와 GitHub repository 검증 전에는 기능 동작 여부를 확정할 수 없다.
...
다음 단계는 Hermes Agent 공식 문서와 GitHub repository를 별도 source로 ingest하고, CLI-only 설치부터 작은 sandbox에서 실행/보안 경계를 확인하는 것이다.
