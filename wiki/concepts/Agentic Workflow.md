...
- [[OpenAI Next Phase of Enterprise AI Article]]은 enterprise agentic workflow가 개별 task assistant에서 여러 agent와 company system을 가로지르는 업무 위임으로 이동한다고 주장한다. 이 단계에서는 workflow 자체뿐 아니라 context, identity, permission, persistent state, audit를 제공하는 [[기업 AI 운영 레이어]]가 필요하다.
- [[Anthropic Claude Fable 5 and Mythos 5 Announcement]]은 장기 agentic workflow에서 model capability뿐 아니라 classifier fallback, persistent file memory, 30일 data retention, trusted access가 runtime behavior와 도입 경계를 결정하는 사례를 제공한다.
- [[Codingsmu LLM as a Judge Article]]은 agent output을 pairwise, single-answer, reference-guided 방식으로 평가하는 [[LLM-as-a-Judge]]를 소개한다. Agent workflow에서 verifier를 LLM으로 분리할 수 있지만, position, verbosity, self-enhancement bias와 judge capability ceiling 때문에 deterministic test, reference, human calibration을 함께 둬야 한다.
- [[PyTorchKR AI ML Papers 2026 06 08 14]]는 Economy of Minds, Self-Harness, AutoForge, AutoScientists, LiveBrowseComp 같은 논문 큐레이션을 통해 agent workflow 연구가 자율 개선, 분산 협업, 환경 합성, 검색 검증, 장기 실험 자동화로 갈라지고 있음을 보여준다. 각 방향은 같은 "agent"라도 필요한 harness, evaluation, human review point가 다르다.
- [[Claude Founders Playbook PDF]]는 [[AI-native Startup]]에서 founder가 individual contributor보다 agent, tool, workflow의 orchestrator가 된다고 설명한다. 이 관점은 agentic workflow를 "자동 실행"이 아니라 stage별 exit criteria, scope, measurement, security, founder attention boundary를 가진 운영 시스템으로 봐야 한다는 사례다.
- [[Prefect Official Site]]는 agent workflow가 반복 실행, worker, observability, auth, governance를 갖춘 production workflow로 운영될 수 있음을 보여준다.
- [[Tavily Official Site]]는 agent가 웹을 조사할 때 real-time search뿐 아니라 extraction, chunking, caching, validation, provenance가 함께 필요하다는 web access layer 사례를 제공한다.
- [[Google Cloud Open Knowledge Format Article]]은 agent가 조직 지식을 매번 흩어진 surface에서 조립하지 않도록 Markdown/YAML bundle을 공유 context로 제공하는 [[Open Knowledge Format]]을 제안한다. Agent workflow에서는 이 bundle을 읽는 규칙뿐 아니라 갱신 권한, review, log, provenance가 함께 필요하다.
- [[Anthropic Claude Tag Announcement]]은 agentic workflow가 개인 chat이나 IDE를 넘어 Slack 채널의 shared work queue로 이동하는 사례다. channel-scoped memory, delegated tool access, task requester, thread result, spend limit, audit log가 팀 agent workflow의 기본 artifact가 된다.
- [[GeekNews Framein Show GN]]은 여러 terminal coding agent를 번갈아 쓸 때 handoff 요약보다 repository-local 작업 계약, ADR, 검증 결과, risk/ship gate를 공유하는 방식이 필요하다고 주장한다. 이 관점은 agentic workflow의 state를 모델 memory가 아니라 사람이 검토 가능한 repo artifact로 남겨야 한다는 기준을 보강한다.
...
- agent output 평가에서 deterministic check, LLM judge, human review를 어떤 위험도 기준으로 조합할 것인가?
- 자율 개선 agent의 instruction 수정은 어떤 회귀 테스트와 승인 경계를 통과해야 실제 workflow에 반영할 수 있는가?
- 웹 검색 agent가 남겨야 할 최소 evidence는 query, source URL, extraction timestamp, content hash, validation result 중 어디까지인가?
- Agent가 OKF bundle을 직접 갱신할 때 어떤 diff, lint, human review gate를 통과해야 하는가?
- Slack 같은 협업 채널에서 agent가 proactive하게 개입할 때 task completion, noise, privacy boundary를 어떻게 측정하고 제한할 것인가?
- 여러 coding agent가 같은 작업을 이어갈 때 작업 계약, ADR, verification, capsule 중 어느 artifact를 최소 상태로 요구할 것인가?
