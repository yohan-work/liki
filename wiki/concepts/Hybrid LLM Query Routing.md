...
- 로컬 소형 모델을 "무료"로 보고 어려운 coding task까지 맡긴 뒤 실패 감지와 fallback을 하지 않는다.
...
- [[로컬 LLM]]: 로컬 모델은 routing 후보가 될 수 있지만, API 비용 대신 latency, 전력, 발열, setup 유지보수, 실패 후 human review 비용을 함께 계산해야 한다.
...
- 로컬 모델은 taxonomy 후보나 형식 변환처럼 실패 비용이 낮은 단계에 먼저 배치하고, source-backed 판단이나 code review는 frontier model로 fallback한다.
...
- 로컬 LLM을 routing 후보에 넣을 때 hardware 상태, timeout, task success rate를 어떤 telemetry로 기록할 것인가?
