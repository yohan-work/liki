...
- [[GeekNews Framein Show GN]]은 [[Framein]]을 여러 coding agent 사이의 handoff 도구가 아니라 repository-local work-state layer로 소개한다. 이는 orchestration이 agent를 나누어 호출하는 문제뿐 아니라 작업 계약, 결정 기록, 검증 결과, gate를 어떤 정본 artifact로 공유할지의 문제임을 보여준다.
...
- 여러 coding agent를 병행할 때 handoff summary와 shared work-state artifact 중 무엇을 정본으로 둘 것인가?
