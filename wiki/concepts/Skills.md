...
- [[GitHub Ponytail Repository]]는 skill을 절차 추가뿐 아니라 과잉 구현을 억제하는 판단 ladder와 review gate로 구성한다. 다만 코드 최소화는 validation, security, accessibility 같은 품질 경계를 보존하고 correctness·rework 평가와 함께 검증해야 한다.
- [[Velog 자바스크립트의 진짜 새로운 변화 번역글]]은 coding assistant에 최신 JavaScript 기능 lookup table을 제공하는 skill을 제안한다. 이런 지식 갱신 skill은 API 이름을 고정하는 대신 공식 source, 확인일, target runtime, compatibility fallback을 함께 관리해야 stale guidance를 줄일 수 있다.
...
- 최소화 skill이 필요한 abstraction과 과잉 abstraction을 구분하는 평가 기준은 무엇인가?
- 빠르게 변하는 언어·runtime 지식을 skill에 넣을 때 freshness와 compatibility를 어떻게 검증할 것인가?
