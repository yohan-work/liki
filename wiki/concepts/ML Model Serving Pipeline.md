...
[[DEVOCEAN Palantir FDE Article]]은 모델, 데이터, PoC가 준비되어도 실제 운영 변화가 생기지 않을 수 있다고 설명한다. Serving pipeline은 배포 자체뿐 아니라 현업 workflow 통합, monitoring, adoption까지 연결되어야 한다.
...
[[Seungbeomdo Hugging Face Pipeline Article]]은 무거운 serving framework 전 단계에서 Transformers `pipeline`으로 pre-trained model을 빠르게 불러오거나 로컬 모델 경로를 지정해 추론하는 경량 실험 흐름을 보여준다.
...
- Productionization 이후 사용자 교육, 업무 프로세스 연결, adoption metric을 설계하지 않는다.
...
- [[Hugging Face]]: 모델 탐색, 빠른 추론 실험, fine-tuning 실험의 출발점이 될 수 있지만 production serving과 동일하게 보면 안 된다.
- [[Forward Deployed Engineer]]: FDE는 serving pipeline을 조직의 실제 운영 프로세스와 연결하는 embedded execution role로 볼 수 있다.
- [[CI/CD 배포 파이프라인]]: 일반 애플리케이션 배포와 모델 serving 모두 artifact, deploy target, verification, rollback을 관리해야 한다.
...
- 모델 serving PoC를 VM에 직접 배포한다면 Jenkins/GitHub Actions 같은 CI/CD와 serving telemetry를 어디까지 연결해야 하는가?
