...
- Remote endpoint, DNS, subdomain, public repository config, 로그에 노출된 token처럼 [[OSINT]]로 발견 가능한 단서를 점검하지 않는다.
...
- [[OSINT]]: Remote MCP endpoint와 관련 설정은 외부에서 관찰 가능한 attack surface가 될 수 있으므로 공개정보 노출 점검 대상이다.
