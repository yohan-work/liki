...
- [[구조화된 답변 프레임워크]]는 query answer와 concept 설명에서 결론, 근거, 예시, 다음 행동을 빠르게 전달하는 보조 구조로 쓸 수 있다. 다만 source summary의 provenance, 검증 필요 주장, 해석 분리는 OREO나 PREP 구조보다 우선한다.
- [[Brunch AI Winning Proposal Claude Bloom Article]]은 최종 제안서 파일만으로는 생성 과정의 암묵지가 남지 않는다고 지적한다. LLM Wiki는 결과물뿐 아니라 source, discovery, 판단, 수정, 실패, 후속 결과를 분리해 보존하는 process memory로도 사용해야 한다.
- [[Google Cloud Open Knowledge Format Article]]은 LLM-wiki pattern을 [[Open Knowledge Format]]이라는 portable Markdown/YAML specification으로 formalize한다고 설명한다. 이 위키는 OKF와 유사한 file-first 구조를 쓰지만, private sensitivity와 source provenance field가 더 강하므로 내부 schema와 외부 export schema를 구분하는 편이 안전하다.
- [[Thinkingsian LLM Wiki와 본유적 부하 Article]]은 사람이 Obsidian 기반 PKM을 LLM Wiki처럼 쓸 때 AI가 링크 선택과 핵심 개념 추출을 대신하면 학습에 필요한 본유적 부하까지 줄일 수 있다고 지적한다. 이 위키에서는 Codex가 raw 보존, provenance, frontmatter, lint 같은 외재적 부하를 줄이되, 사용자가 연결 판단, 핵심 개념 명명, 재요약, 후속 질문 선택 같은 이해 형성 과정을 완전히 위임하지 않도록 구분해야 한다.
...
- OKF-compatible export를 만들 때 어떤 field를 유지하고 어떤 private field를 제거해야 하는가?
