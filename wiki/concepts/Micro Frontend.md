...
- [[CSS 구조화]]: reset, component, token, utility, spacing 같은 style codebase layer의 책임을 나누는 방식이다. Micro Frontend는 이보다 큰 application/deployment boundary를 다루지만, 여러 frontend unit이 공존할 때 CSS layer 규칙은 shared contract가 된다.
...
- 여러 frontend unit이 독립 배포될 때 Tailwind utility, shared token, component CSS의 ownership을 어떻게 나눌 것인가?
