...
- [[JobKorea FDE Role and Interview Guide]]는 국내외 공고에서 코딩, 고객 문제 정의, 이해관계자 커뮤니케이션이 함께 반복된다고 정리한다. 다만 공고 통계는 외부 분석의 재인용이므로 방향성 신호로만 사용한다.
...
- [[Brunch Developer to Sales Career Article]]은 18년 경력 개발자가 solutions engineer로 옮기며 PoC와 hands-on session으로 고객 문제와 제품 사용법을 연결하려는 사례를 전한다. 이 역할은 FDE와 인접하지만, 실제 production code ownership과 제품 capability 환류가 약하면 FDE보다 technical sales 또는 solutions engineering에 가깝다.
...
- 직함보다 문제 정의, 구현, 프로덕션 배포, 검증, 개선의 실제 비중을 확인한다.
- 제품팀으로 feedback이 돌아가고 고객별 prototype이 재사용 가능한 asset으로 승격되는 구조가 있는지 본다.
- 제안 발표, 데모, 교육, 유지보수만 강조되고 코드 ownership이 약하면 solutions engineering 또는 customer support 역할일 수 있다.
- PoC와 hands-on session을 강조하더라도 고객별 구현을 production으로 책임지고 제품팀에 환류하는 구조가 없다면 FDE보다 solutions engineer에 가깝다.
- 외부 고객이 아니라 내부 조직을 대상으로 하는 Internal AX형도 가능하므로 customer의 범위를 확인한다.
- 지원 경험은 문제 정의, 의사결정, 결과, tradeoff로 정리해 기능 목록보다 판단과 end-to-end ownership을 드러낸다.
