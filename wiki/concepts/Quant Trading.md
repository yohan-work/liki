...
- [[토스증권 Open API]]는 국내/미국 주식의 시장 데이터, 종목 정보, 계좌 상태, 주문 실행 API를 제공하므로 quant trading PoC에서 data retrieval, account state, execution layer를 분리해 생각하게 해주는 공식 API 사례다.
- [[레버리지 ETF 변동성 끌림]]은 전략 수익률이 단순 방향성뿐 아니라 가격 경로, 변동성, 일일 리밸런싱 구조에 의존한다는 점을 보여준다.
- [[TimesFM]] 같은 time-series foundation model은 zero-shot forecasting baseline 후보가 될 수 있지만 일반 dataset 성능을 trading alpha로 해석하면 안 된다. 투자 적용에는 target 정의, walk-forward split, leakage 방지, statistical baseline, quantile calibration, transaction cost와 regime별 error가 필요하다.
...
- read-only 시장 데이터 자동화와 실제 주문 자동화 사이의 approval boundary를 어떻게 설계해야 하는가?
- 레버리지 ETF 전략은 단순 수익률 backtest 외에 volatility decay와 path dependency를 어떻게 평가해야 하는가?
- 시계열 foundation model이 seasonal naive나 단순 statistical model보다 실제 투자 research에 추가하는 정보가 있는가?
