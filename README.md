...
Obsidian에서 먼저 볼 화면은 `wiki/overview.md`입니다. 이 페이지는 다음 행동, 반복 루틴, 주요 진입점을 모아 둔 운영 대시보드이고, `wiki/index.md`는 전체 목차로 사용합니다.
...
5. Obsidian의 `LLM Wiki/wiki/index.md`에서 새 링크와 요약을 확인합니다.
6. 터미널에서 `git diff`로 변경 내용을 검토합니다.
7. 문제가 없으면 commit하고, 필요하면 GitHub remote로 push합니다.
